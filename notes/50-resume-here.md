...
## Sunday 20 September — done

- **The post-build gate exists.** `scripts/check-built.py`, wired into
  `npm run build`, reads `site/dist` and fails if a selector a style block
  declares never reaches the shipped CSS. It looks in all three places CSS can
  arrive: the hashed bundle, the plain stylesheet copied out of `public/`, and
  a block inlined into the page. My first cut read only the first and
  immediately declared the nav broken when it was fine — the same mistake,
  inside the check written to prevent it. Negative-tested.
- **The church-records filter is live**, and generalised to seven record
  kinds. One shared classifier, `scripts/record_kinds.py`, reads the kind out
  of a collection's title; 682 of 3,489 FamilySearch collections are church
  registers and 389 places hold one. Carried on the index as a bitmask so it
  filters on first paint. `?kind=church` round-trips.
- **David's contributed directory is in.** 969 of 1,399 rows on the country
  pages, grouped by kind. 430 dropped for dead links.

## Still open

- **Antenati is stuck on THEIR side, not ours.** The SAN OAI endpoint
  (`www.san.beniculturali.it`, 2.42.228.36) is unreachable — port 80 refuses,
  every request times out, and `san.beniculturali.it` itself is down too. The
  harvest is stopped cleanly at **5,591 volumes / 6,000 of 464,770 images**
  with its resumption token intact in `data/antenati-registers.json`. Retry
  `python3 scripts/harvest-antenati-oai.py --resume` when the Ministry's
  server is back; test it first with
  `curl "http://www.san.beniculturali.it/oaicat/OAIHandler?verb=Identify"`.
- **recordatlas.org is bought but points at the registrar**, not GitHub:
  A records are 15.197.225.128 and 3.33.251.168, serving a landing page.
  `scripts/cutover-domain.sh` refuses to run until at least two of GitHub's
  four addresses appear, and does the whole switch when they do. David has to
  replace the A records with 185.199.108–111.153.
- Promote the Italian comuni before registers can land on them.
- Croatia's surnames via the PxWeb catalogue.
- **The ad is code-complete and unfilled.** The slot measures 1024x90, the
  loader runs, Google answers `data-ad-status="unfilled"` — an account state.
  David to check AdSense → Sites for approval.
- Waiting on David: NARA API key, Trove reply (RSref189042).

## Two lessons worth keeping

**Inheriting a stylesheet means inheriting the markup it was written for.**
styles.css came across whole from the family archive with a complete
navigation design — dropdown groups, a menu button, a drawer — and this
layout rendered eleven bare links the sheet had no rule for.

**A check that looks in one of the places a thing can be is not a check.**
Three wrong diagnoses stayed alive because `grep topnav dist/_astro/*.css`
returns zero however healthy the CSS is.
